...
public class FixedMoveControl extends MovementController {
    ...
    public void tick() {
        if (this.action == Action.STRAFE) {
            float attrSpeed = (float) this.mob.getAttributeValue(Attributes.MOVEMENT_SPEED);
    ...
    protected void checkStrifeToPos(float speed) {
        ...
        if (!this.canWalkable(strifeToPos)) {
            ...
            if (!strifeToPos.equals(entityPos) && this.canWalkable(entityPos)) {
                Vector2f strife = getPosToStrife(entityPos.getX() + 0.5F, entityPos.getZ() + 0.5F);
                ...
                for (int i = -1; i <= 1; i++) {
                    for (int j = -1; j <= 1; j++) {
                        ...
                        if (canWalkable(checkPos)) {
                            Vector2f strife = getPosToStrife(checkPos.getX() + 0.5F, checkPos.getZ() + 0.5F);
                            this.moveForward = strife.x;
                            this.moveStrafe = strife.y;
                        }
                    }
    ...
    protected Vector2f getPosToStrife(float x, float z) {
        ...
        return new Vector2f(forward, sideways);
    }
