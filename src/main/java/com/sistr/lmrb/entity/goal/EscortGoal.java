...
public class EscortGoal extends Goal {
    ...
    public EscortGoal(CreatureEntity escort, ITameable tameable, float minDistance, float maxDistance) {
        this.escort = escort;
    ...
    public void tick() {
        ...
        if (--this.timeToRecalcPath <= 0) {
            ...
            if (!this.escort.getLeashed() && !this.escort.isPassenger()) {
                if (this.escort.getDistanceSq(this.owner) >= 12 * 12) {
                    this.tryTeleport();
                } else {
                    this.navigator.tryMoveToEntityLiving(this.owner, 1);
                }
